...
void print_ascii_art(const char* ascii_file, const char* color) {
    char* home = getenv("HOME");
    char file_path[512];
...
void print_system_info(system_info_t* info, config_t* config) {
    ...
    printf("\n");
    printf("%s%s@%s%s\n", BOLD, info->username, info->hostname, RESET);
    ...
    if (config->show_os) {
        printf("%sOS:%s        %s\n", color, RESET, info->distro);
    }
    if (config->show_kernel) {
        printf("%sKernel:%s    %s\n", color, RESET, info->kernel);
    }
    if (config->show_uptime) {
        printf("%sUptime:%s    %s\n", color, RESET, info->uptime);
    }
    if (config->show_packages) {
        printf("%sPackages:%s  %s\n", color, RESET, info->packages);
    }
    if (config->show_shell) {
        printf("%sShell:%s     %s\n", color, RESET, info->shell);
    }
    if (config->show_terminal) {
        printf("%sTerminal:%s  %s\n", color, RESET, info->terminal);
    }
    
    if (config->show_wm && strcmp(info->compositor, "Unknown") != 0) {
        printf("%sDE/WM:%s     %s on %s\n", color, RESET, info->wm, info->compositor);
    }
    
    if (config->show_cpu) {
        printf("%sCPU:%s       %s\n", color, RESET, info->cpu);
    }
    
    if (config->show_gpu && strcmp(info->gpu, "Unknown") != 0) {
        printf("%sGPU:%s       %s\n", color, RESET, info->gpu);
        if (strcmp(info->gpu_driver, "Unknown") != 0) {
            printf("%sGPU Driver:%s %s\n", color, RESET, info->gpu_driver);
        }
        if (strcmp(info->gpu_memory, "Unknown") != 0) {
            printf("%sGPU Memory:%s %s\n", color, RESET, info->gpu_memory);
        }
    }
    
    if (config->show_memory) {
        printf("%sMemory:%s    %s\n", color, RESET, info->memory);
    }
    if (config->show_disk) {
        printf("%sDisk:%s      %s\n", color, RESET, info->disk);
    }
    
    if (config->show_resolution && strcmp(info->resolution, "Unknown") != 0) {
        printf("%sResolution:%s %s\n", color, RESET, info->resolution);
    }
    
    if (config->show_colors) {
        printf("\n%sColors:%s ", color, RESET);
